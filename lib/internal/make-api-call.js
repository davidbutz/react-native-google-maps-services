...
exports.inject = function(options) {
   ...
   * }
   *
   * @param {{ url: string, baseURL: string, relativeURL: string }} path
  ...
  return function(path, query, callback) {
    ...
    var requestUrlConfig = formatRequestUrl(path.baseURL, path.relativeURL, isPost ? {} : query, useClientId);

    if (isPost) {
      queryOptions.body = query;
      queryOptions.method = 'post';
    } else {
    ...
    function rateLimitedGet() {
      return requestQueue.add(function() {
        return Task.start(function(resolve, reject) {
          return makeUrlRequest(requestUrlConfig, resolve, reject, queryOptions);
        });
      });
    }
    ...
    var task =
        ...
        .thenDo(function(response) {
          ...
          response.requestUrl = requestUrlConfig;
   ...
   * @return {{ url: string, baseURL: string, params: Object }} Partial configuration for axios.
   */
  function formatRequestUrl(baseURL, relativeURL, query, useClientId) {
    if (channel) {
    ...
    if (useClientId) {
      ...
      var queryString = Object.keys(query)
        ...
        .join('&');
      var payload = relativeURL + '?' + queryString;
  ...
  function computeSignature(secret, payload) {
    var hash = CryptoJS.HmacSHA1(payload.toString(), secret.toString());
